...
defmodule DotTest do
  ...
  test "graph with one colored edge" do
    assert %Graph{edges: [{:a, :b, [color: :blue]}]} ==
             exprt(
               Dot.graph do
                 a -- b(color: :blue)
               end
  ...
  test "non-keyword attribute list in graph" do
    assert_raise ArgumentError, fn ->
      exprt(
        Dot.graph do
          graph(["Alpha!", color: green])
        end
      )
    end
  end

  test "non-keyword attribute list in edge" do
    assert_raise ArgumentError, fn ->
      exprt(
        Dot.graph do
          a -- b(["Alpha!", color: green])
        end
      )
    end
  end
