defmodule Meetup do
  ...
  def meetup(year, month, weekday, :first),
    do: next_weekday(year, month, 1, weekday_to_integer(weekday))

  def meetup(year, month, weekday, :second),
    do: next_weekday(year, month, 8, weekday_to_integer(weekday))

  def meetup(year, month, weekday, :third),
    do: next_weekday(year, month, 15, weekday_to_integer(weekday))

  def meetup(year, month, weekday, :fourth),
    do: next_weekday(year, month, 22, weekday_to_integer(weekday))

  def meetup(year, month, weekday, :teenth),
    do: next_weekday(year, month, 13, weekday_to_integer(weekday))
